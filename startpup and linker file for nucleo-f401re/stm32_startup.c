...
void Reset_Handler(void); // .elf pirma funkcija
...
uint32_t vectors[] __attribute__((section(".isr_vector"))) = { // nested vector table
	STACK_START,
...
void Reset_Handler(){
	...
	uint32_t size_of_data = ((&_edata) - (&_sdata));
	...
	uint32_t size_of_bss = ((&_ebss) - (&_sbss));
